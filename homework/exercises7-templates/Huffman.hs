...
-----------------------------------------------------------------------
tips :: [(a,Int)] -> [(Btree a, Int)]
tips = sortBy (comparing snd) . map (\(c,f) -> (Tip c, f))

step :: [(Btree a, Int)] -> [(Btree a, Int)]
step [] = []
step [p] = [p]
step ((t1,i):(t2,j):ps) = step (insertBy (comparing snd) (Bin t1 t2, i+j) ps)

huffman :: [(a,Int)] -> Btree a
huffman = fst . head . step . tips

-----------------------------------------------------------------------

codes :: Btree a -> [(a,[Bit])]
codes (Bin t1 t2) = (map (\(x,y) -> (x,O:y)) (codes t1)) ++ (map (\(x,y) -> (x,I:y)) (codes t2))
codes (Tip x)     = [(x,[])]

encode :: (Ord a) => Btree a -> [a] -> [Bit]
encode t [] = []
encode t (c:cs) = case find (\x -> fst x == c) (codes t) of 
  Nothing -> error "Could not find element in code tree"
  Just (_,code) -> code ++ encode t cs
  
-----------------------------------------------------------------------

decode :: (Ord a) => Btree a -> [Bit] -> [a]
decode t txt = decoding t t txt where
  decoding _ _ [] = []
  decoding tr (Tip a) txt = a:(decoding t t txt)
  decoding tr (Bin t1 t2) (bit:txt) = case bit of
    O -> decoding tr t1 (txt) 
    I -> decoding tr t2 (txt)
